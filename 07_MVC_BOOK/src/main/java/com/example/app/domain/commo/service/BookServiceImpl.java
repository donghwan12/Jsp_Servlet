...
import com.example.app.domain.commo.dao.BookDao;
import com.example.app.domain.commo.dao.BookDaoImpl;
...
import com.example.app.domain.commo.dto.PageDto;
...
public class BookServiceImpl implements BookService {
	...
	public Map<String,Object> getAllBooks(Criteria criteria)  throws Exception{
		...
		if(criteria.getType()==null||criteria.getKeyword()==null) { //타입과 키보드가 없을경우
			int count =  dao.Count();
			...
			resultValue.put("pageDto", pageDto); //페이징처리
			resultValue.put("list", list);	//해당 페이지에 표시할 게시물들
			resultValue.put("count",count); //해당페이지 건수
		}
		else
		{
